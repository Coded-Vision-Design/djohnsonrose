...
// Win11-style speaker icon with three wave fragments — fades waves out as volume
// drops, matching partials/volume-icon.php.
export function VolumeIcon({ className = 'w-4 h-4' }: Props) {
  const volume = useOsStore((s) => s.settings.volume)
  ...
  return (
    <svg
      ...
      aria-label={muted ? 'Volume muted' : `Volume ${volume}%`}
    >
      <path
      ...
      />
      <path
        d="M14.0814 12.933C13.8421 12.7952 13.7599 12.4894 13.8977 12.2502C14.6739 10.9026 14.7294 9.19135 13.897 7.74967C13.759 7.51052 13.8409 7.20473 14.08 7.06665C14.3192 6.92858 14.625 7.01052 14.7631 7.24967C15.7811 9.01294 15.7112 11.1052 14.7642 12.7493C14.6264 12.9886 14.3207 13.0708 14.0814 12.933Z"
        fill="currentColor"
        className={volume < 1 ? 'opacity-20' : ''}
      />
      <path
        ...
        fill="currentColor"
        className={volume < 66 ? 'opacity-20' : ''}
      />
    </svg>
  )
}
