...
<div class="h-12 w-full taskbar-glass flex items-center justify-between px-2 z-9999 border-t border-white/20"
    ...
    <div class="flex items-center w-48 overflow-hidden" x-show="isDesktop" @contextmenu.prevent="showTaskbarContextMenu($event)">
        <div @click="toggleWidgets()" class="px-2 py-1.5 rounded-sm hover:bg-white/10 cursor-pointer flex items-center space-x-2 transition-colors group">
            <span class="text-xl filter drop-shadow-xs group-hover:scale-110 transition-transform" x-text="weather.icon"></span>
    ...
    <div class="flex items-center space-x-1" :class="{ 'taskbar-center-mobile': !isDesktop }">
        ...
        <button x-cloak @click="startMenuOpen = !startMenuOpen; playSound('click')" 
                class="w-10 h-10 flex items-center justify-center rounded-sm hover:bg-white/10 transition-colors group"
        ...
        <button x-show="isDesktop" @click="startMenuOpen = true; $nextTick(() => document.querySelector('input[x-model=\'searchQuery\']').focus())" class="w-10 h-10 flex items-center justify-center rounded-sm hover:bg-white/10 transition-colors">
        ...
        <button x-show="!isMobile" @click="openApp('taskmanager')" class="w-10 h-10 flex items-center justify-center rounded-sm hover:bg-white/10 transition-colors">
        ...
        <template x-for="appId in [...new Set([...$store.os.pinnedApps, ...windows.map(w => w.app)])]" :key="appId">
            <button @click="openApp(appId)" 
                    ...
                    class="w-10 h-10 flex items-center justify-center rounded-sm hover:bg-white/10 transition-all relative group"
                    :class="{ 'bg-white/10 shadow-inner': windows.some(w => w.app === appId && focusedWindowId === w.id) }">
                
                <div class="w-7 h-7 rounded-sm flex items-center justify-center bg-white/10 text-[10px] font-bold text-gray-700 dark:text-white overflow-hidden transition-transform group-hover:scale-110">
    ...
    <div class="flex flex-row items-center space-x-1 px-2 text-black dark:text-white" 
        ...
        <div x-show="hiddenIconsOpen" 
            ...
            <template x-if="windows.some(w => w.app === 'outlook')">
                <div class="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-sm cursor-pointer transition-colors flex items-center justify-center" @click="openApp('outlook'); hiddenIconsOpen = false" title="Outlook">
                    <img :src="window.portfolioConfig.imgPath + 'outlook.webp'" class="w-5 h-5">
                </div>
            </template>
            <template x-if="windows.some(w => w.app === 'pdfreader')">
                <div class="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-sm cursor-pointer transition-colors flex items-center justify-center" @click="openApp('pdfreader'); hiddenIconsOpen = false" title="PDF Reader">
                    <img :src="window.portfolioConfig.imgPath + 'pdf.webp'" class="w-5 h-5">
                </div>
            </template>
            <template x-if="windows.some(w => w.app === 'vscode')">
                <div class="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-sm cursor-pointer transition-colors flex items-center justify-center" @click="openApp('vscode'); hiddenIconsOpen = false" title="Visual Studio Code">
                    <img :src="window.portfolioConfig.imgPath + 'vscode.webp'" class="w-5 h-5">
                </div>
            </template>
            <template x-if="settings.bluetooth">
                <div class="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-sm cursor-pointer transition-colors flex items-center justify-center" title="Bluetooth Devices">
                    <img :src="window.portfolioConfig.imgPath + 'bluetooth.webp'" class="w-5 h-5 dark:invert opacity-80">
                </div>
            </template>
            
            <!-- Constant Tray Icons -->
            <div class="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-sm cursor-pointer transition-colors flex items-center justify-center" title="Windows Security">
                <img :src="window.portfolioConfig.imgPath + 'win11/view.webp'" class="w-5 h-5 dark:invert opacity-70">
            </div>
            <div class="p-2 hover:bg-black/5 dark:hover:bg-white/10 rounded-sm cursor-pointer transition-colors flex items-center justify-center" @click="openApp('settings'); hiddenIconsOpen = false" title="Settings">
        ...
        <button @click="quickSettingsOpen = !quickSettingsOpen; playSound('click')" 
            ...
            <div class="flex flex-row items-center relative" :title="`${settings.batteryLevel}% ${settings.isCharging ? '(Charging)' : ''}`">
                ...
                <template x-if="settings.batterySaver && !settings.isCharging">
                    <div class="absolute -top-0.5 -right-0.5 w-1.5 h-1.5 rounded-full border border-white dark:border-win-dark shadow-xs bg-yellow-500"></div>
                </template>
            </div>
        </button>
        ...
        <a href="/v2/"
           ...
           class="hidden md:flex items-center px-2 py-1 mr-1 rounded-sm text-[10px] font-medium border border-win-blue/30 bg-win-blue/10 text-win-blue hover:bg-win-blue hover:text-white transition-colors">
            ⚛ React v2
        </a>

        <!-- Clock/Date -->
        <button class="px-2 py-1 rounded-sm hover:bg-white/10 text-xs text-right leading-tight transition-colors" @click="startMenuOpen = false; quickSettingsOpen = false">
