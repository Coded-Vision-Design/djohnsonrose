<!-- partials/volume-icon.php -->
<svg class="<?php echo $className ?? ''; ?>" 
     viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg" 
     x-data="{ 
        get volume() { return $store.os.settings.volume },
        get isMuted() { return this.volume == 0 }
     }">
    <!-- Speaker Body (Official Win11 Path Fragment) -->
    <path d="M11.9971 3.00588C11.9971 2.13286 10.9568 1.67903 10.3168 2.27288L6.44467 5.86626C6.3522 5.95208 6.23071 5.99976 6.10456 5.99976H3.49707C2.66864 5.99976 1.99707 6.67134 1.99707 7.49976V12.4998C1.99707 13.3282 2.66864 13.9998 3.49707 13.9998H6.10456C6.23071 13.9998 6.3522 14.0474 6.44467 14.1333L10.3168 17.7266C10.9568 18.3205 11.9971 17.8667 11.9971 16.9936V3.00588Z" 
          fill="currentColor"
          :class="isMuted ? 'opacity-40' : ''" />
    
    <!-- Wave 1 (Inner) - Active > 0 -->
    <path d="M14.0814 12.933C13.8421 12.7952 13.7599 12.4894 13.8977 12.2502C14.6739 10.9026 14.7294 9.19135 13.897 7.74967C13.759 7.51052 13.8409 7.20473 14.08 7.06665C14.3192 6.92858 14.625 7.01052 14.7631 7.24967C15.7811 9.01294 15.7112 11.1052 14.7642 12.7493C14.6264 12.9886 14.3207 13.0708 14.0814 12.933Z" 
          fill="currentColor"
          :class="volume < 1 ? 'opacity-20' : ''" />
    
    <!-- Wave 2/3 (Outer) - Active > 33 / 66 -->
    <!-- We split the official outer wave into two for better visual scaling -->
    <path d="M15.2567 4.62735C15.4623 4.44308 15.7785 4.46042 15.9627 4.66609C18.679 7.69768 18.679 12.3018 15.9628 15.3334C15.7785 15.5391 15.4624 15.5564 15.2567 15.3721C15.051 15.1879 15.0337 14.8718 15.218 14.6661C17.5939 12.0143 17.5939 7.98519 15.2179 5.3334C15.0337 5.12773 15.051 4.81162 15.2567 4.62735Z" 
          fill="currentColor"
          :class="volume < 66 ? 'opacity-20' : ''" />
</svg>
