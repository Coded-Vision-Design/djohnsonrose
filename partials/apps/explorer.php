...
<div class="h-full flex flex-col bg-white dark:bg-[#1c1c1c] text-black dark:text-white select-none" x-data="explorerApp()">
    ...
    <div class="h-12 border-b border-gray-200 dark:border-gray-800 flex items-center px-2 space-x-4 bg-white dark:bg-[#2b2b2b]">
        <div class="flex items-center space-x-1 pr-2 border-r border-gray-200 dark:border-gray-700">
            <button @click="goBack()" :disabled="historyStep <= 0" class="p-1.5 rounded-sm hover:bg-gray-100 dark:hover:bg-white/5 disabled:opacity-30"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg></button>
            <button @click="goForward()" :disabled="historyStep >= history.length - 1" class="p-1.5 rounded-sm hover:bg-gray-100 dark:hover:bg-white/5 disabled:opacity-30"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></button>
            <button class="p-1.5 rounded-sm hover:bg-gray-100 dark:hover:bg-white/5"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 10l7-7m0 0l7 7m-7-7v18"></path></svg></button>
        </div>
        
        <div class="grow flex items-center bg-gray-100 dark:bg-black/20 rounded-md px-3 py-1.5 text-xs border border-gray-200 dark:border-white/10 shadow-inner overflow-hidden relative cursor-text min-h-[32px]"
            ...
            <div x-show="addressBarActive" class="w-full h-full flex items-center">
                <input type="text" 
                       ...
                       class="bg-transparent border-none outline-hidden w-full text-xs font-mono">
            </div>
        </div>
        ...
        <div class="w-32 sm:w-64 bg-gray-100 dark:bg-black/20 rounded-md px-3 py-1.5 text-xs border border-gray-200 dark:border-white/10 flex items-center focus-within:bg-white dark:focus-within:bg-black/40 transition-all relative">
            <svg class="w-3.5 h-3.5 text-gray-400 mr-2 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg>
            <input type="text" x-model="searchQuery" placeholder="Search" class="bg-transparent border-none outline-hidden w-full truncate pr-6">
    ...
    <!-- Main Content Area -->
    <div class="flex grow overflow-hidden bg-white dark:bg-[#1c1c1c]">
        <!-- Sidebar -->
                ...
                <div>
                    ...
                    <template x-if="item.type !== 'header'">
                        <button @click="navigate(item.path)" 
                                ...
                                class="w-full text-left py-1.5 rounded-sm hover:bg-gray-100 dark:hover:bg-white/5 flex items-center group transition-colors">
        ...
        <!-- File Pane -->
        <div class="grow p-4 overflow-y-auto bg-white dark:bg-[#1c1c1c]">
            ...
            <div class="grid gap-4 grid-cols-[repeat(auto-fill,minmax(100px,1fr))]">
                <template x-for="file in currentFiles" :key="file.name">
                    <div @dblclick="openItem(file)" 
                         ...
                         :title="file.name"
                         :class="{'bg-blue-100/50 dark:bg-blue-900/30 border-blue-300 dark:border-blue-700 shadow-xs': selected === file.name}"
                         class="flex flex-col items-center p-2 rounded-sm border border-transparent hover:border-blue-200 dark:hover:border-blue-800 hover:bg-blue-50/50 dark:hover:bg-blue-900/10 cursor-pointer group transition-all w-[100px]">
                        
                        <!-- Thumbnail or Icon -->
                        <div class="w-12 h-12 mb-2 flex items-center justify-center relative overflow-hidden rounded-sm shadow-xs group-hover:scale-105 transition-transform shrink-0">
                            ...
                            <template x-if="file.type !== 'image'">
                                <div class="w-full h-full flex items-center justify-center"
                                    ...
                                    <template x-if="!file.icon || !file.icon.includes('.')">
                                        <div class="text-4xl drop-shadow-xs" x-text="file.icon || '📁'"></div>
                                    </template>
                                    <!-- Status Overlay -->
                                    <template x-if="file.status">
                                        <div class="absolute bottom-0 right-0 bg-white/80 dark:bg-black/80 rounded-full w-5 h-5 flex items-center justify-center text-[10px] shadow-xs border border-black/5 dark:border-white/10" x-text="file.status"></div>
                                    </template>
                                </div>
                            </template>
                        </div>

                        <div class="text-[11px] text-center w-full px-0.5 font-medium leading-tight line-clamp-2 overflow-hidden wrap-break-word text-wrap" x-text="file.name"></div>
                        ...
                        <template x-if="currentPath === 'C:\\Recycle Bin' || searchQuery.length > 0 && file.path === 'C:\\Recycle Bin'">
                            <button @click.stop="restoreItem(file)" 
                                    class="mt-2 px-2 py-0.5 bg-win-blue text-white text-[9px] rounded-sm hover:bg-blue-600 transition-colors">
                                Restore
                            </button>
                        </template>
                    </div>
    ...
    <!-- Status Bar -->
    <div class="h-6 border-t border-gray-200 dark:border-gray-800 bg-win-bg dark:bg-[#2b2b2b] flex items-center px-4 justify-between text-[10px] opacity-60">
