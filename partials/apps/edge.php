...
<div class="h-full flex flex-col bg-white dark:bg-[#1c1c1c] text-black dark:text-white" x-data="edgeApp()">
    ...
    <div class="bg-gray-100 dark:bg-[#2b2b2b] pt-1.5 pb-2 space-y-1.5 shrink-0 border-b dark:border-white/5">
        ...
        <div class="flex items-center space-x-1 px-3 overflow-x-auto scrollbar-none">
            <template x-for="tab in tabs" :key="tab.id">
                <div @click="activeTabId = tab.id" 
                    ...
                    <button @click.stop="closeTab(tab.id)" 
                            ...
                            class="ml-auto hover:bg-gray-300 dark:hover:bg-white/10 rounded-sm p-0.5 transition-opacity duration-200">
            ...
            <button @click="addTab()" class="p-1.5 rounded-sm hover:bg-gray-200 dark:hover:bg-white/5 shrink-0 ml-1">
        ...
        <div class="flex items-center space-x-1 sm:space-x-2 px-3 mt-[2px]">
            <div class="flex items-center space-x-0.5">
                <button class="p-2 rounded-sm hover:bg-gray-200 dark:hover:bg-white/5 hidden sm:block opacity-70"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 19l-7-7m0 0l7-7m-7 7h18"></path></svg></button>
                <button class="p-2 rounded-sm hover:bg-gray-200 dark:hover:bg-white/5 hidden sm:block opacity-70"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path></svg></button>
                <button @click="refreshPage()" class="p-2 rounded-sm hover:bg-gray-200 dark:hover:bg-white/5 transition-transform" :class="{ 'animate-spin': isRefreshing }">
            ...
            <div class="grow bg-white dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-full px-4 py-1 flex items-center shadow-xs min-w-0 h-8 group focus-within:ring-2 ring-win-blue/20">
                ...
                <input type="text" 
                       ...
                       class="bg-transparent border-none outline-hidden w-full text-xs truncate font-normal" 
                       @keydown.enter="loadUrl()">
            </div>
            <!-- Edge Menu (...) -->
            <div class="relative" x-data="{ open: false }">
                <button @click="open = !open" @click.away="open = false" class="p-2 rounded-sm hover:bg-gray-200 dark:hover:bg-white/5 opacity-70">
    ...
    <!-- Content Area -->
    <div class="grow relative overflow-auto bg-gray-50 dark:bg-[#1c1c1c]">
                            ...
                            <div class="p-6">
                                    ...
                                    <div class="flex items-center space-x-2 px-2 py-1 <?php echo (isset($p['country_code']) && strpos($p['country_code'], 'gb') !== false) ? 'bg-win-blue/10 text-win-blue border-win-blue/20' : 'bg-gray-100 dark:bg-white/5 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-white/10'; ?> rounded-full border">
                                        <?php if (isset($p['country_code'])): ?>
                                            <img src="https://flagcdn.com/w20/<?php echo strtolower($p['country_code']); ?>.png" 
                                                 class="w-4 h-auto rounded-xs shadow-xs"
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <?php foreach ($p['tags'] as $t): ?>
                                        <span class="px-2 py-0.5 bg-gray-100 dark:bg-white/10 rounded-sm text-[10px] text-gray-500 uppercase tracking-wider"><?php echo $t; ?></span>
                                    <?php endforeach; ?>
                                </div>
                            </div>
                        </div>
