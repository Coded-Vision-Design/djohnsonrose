<!-- partials/apps/photos.php -->
    <div class="h-full flex flex-col bg-win-bg dark:bg-[#1c1c1c] text-black dark:text-white select-none" 
         x-data="photosApp()"
    ...
    <div class="h-12 border-b border-gray-200 dark:border-gray-800 flex items-center px-4 justify-between bg-white dark:bg-[#2b2b2b]">
        ...
        <div class="flex items-center space-x-2">
            <button @click="zoomOut()" class="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-sm transition-colors">
            ...
            <button @click="zoomIn()" class="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-sm transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7"></path></svg>
            </button>
            <div class="w-px h-6 bg-gray-200 dark:bg-gray-700 mx-2"></div>
            <button @click="rotate()" class="p-2 hover:bg-gray-100 dark:hover:bg-white/5 rounded-sm transition-colors">
    ...
    <!-- Image Viewer -->
    <div class="grow relative overflow-hidden bg-[#eeeeee] dark:bg-[#0a0a0a] flex items-center justify-center p-8">
    ...
    <div class="h-20 bg-white dark:bg-[#2b2b2b] border-t border-gray-200 dark:border-gray-800 flex items-center px-4 space-x-2 overflow-x-auto scrollbar-none">
        <template x-for="(img, index) in allImages" :key="img.name">
            <div @click="setIndex(index)" 
                 ...
                 class="h-16 w-16 shrink-0 rounded-sm border-2 p-0.5 cursor-pointer transition-all">
                <img :src="img.url" class="w-full h-full object-cover rounded-xs">
            </div>
        </template>
    </div>
</div>
