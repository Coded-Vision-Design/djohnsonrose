...
<div x-show="startMenuOpen" 
    ...
    <div class="p-8 pb-4" :class="{ 'p-4 pb-2': !isDesktop }">
        <div class="relative group">
            <input type="text" 
                   ...
                   class="w-full bg-white dark:bg-black/20 border-b-2 border-win-blue px-10 py-2 rounded-md outline-hidden text-sm transition-all focus:bg-white dark:focus:bg-[#1c1c1c]"
    ...
    <!-- Search Results -->
    <div x-show="searchQuery.length > 0" class="grow overflow-y-auto px-8 custom-scrollbar">
        ...
        <div class="space-y-1">
            <template x-for="item in searchResults" :key="item.name">
                <div @click="openItem(item, item.fullPath); searchQuery = ''; startMenuOpen = false" 
                     ...
                     class="flex items-center p-2 rounded-sm hover:bg-win-blue hover:text-white cursor-pointer group transition-colors">
                    <div class="w-8 h-8 rounded-sm bg-black/5 dark:bg-white/5 flex items-center justify-center mr-3 group-hover:bg-white/20">
                    ...
                    <div class="min-w-0 grow text-black dark:text-white group-hover:text-white">
    ...
    <!-- Pinned Apps View -->
    <div x-show="searchQuery.length === 0 && startMenuView === 'pinned'" class="grow overflow-y-auto" :class="!isDesktop ? 'px-4' : 'px-8'">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-sm font-semibold dark:text-white">Pinned</h2>
            <button @click="startMenuView = 'allApps'" class="text-xs bg-white/20 dark:bg-white/5 px-2 py-1 rounded-sm hover:bg-white/30 dark:hover:bg-white/10 dark:text-gray-300 transition-colors">All apps ></button>
        </div>
        ...
        <div class="grid gap-y-6 gap-x-4 grid-cols-4 sm:grid-cols-6">
            ...
            { id: 'settings', name: 'Settings', icon: window.portfolioConfig.imgPath + 'settings.webp' },
                ...
                <div @click="openApp(app.id)" 
                    ...
                    <div class="w-10 h-10 rounded-sm flex items-center justify-center text-2xl group-hover:scale-110 transition-transform bg-white/5 dark:bg-white/5 group-hover:bg-white/10 shrink-0 overflow-hidden" 
                        ...
                        <template x-if="!app.icon.includes('.')">
                            <div class="w-full h-full flex items-center justify-center">
                                <template x-if="app.id === 'pdfreader'">
                                    <div class="w-5 h-7 bg-red-600 rounded-xs flex items-center justify-center text-[7px] font-bold text-white uppercase">PDF</div>
                                </template>
                                <template x-if="app.id !== 'pdfreader'">
            ...
            <div class="grid grid-cols-1 gap-2">
                <template x-for="file in (function() {
                    ...
                    <div @click="file.name.endsWith('.pdf') ? openApp('pdfreader', file.name, {pdfUrl: file.pdfUrl || (window.portfolioConfig.basePath + 'data/cv.pdf')}) : (file.type === 'app' ? openApp(file.app, file.name) : (file.type === 'image' ? openApp('photos', file.name, {imageName: file.name}) : (file.type === 'video' ? openApp('video', file.name, {videoUrl: file.url}) : openApp('explorer'))))" 
                         ...
                         class="flex items-center p-2 rounded-sm hover:bg-white/20 dark:hover:bg-white/10 cursor-pointer group transition-colors">
                        <div class="w-8 h-8 rounded-sm bg-gray-100 dark:bg-white/5 flex items-center justify-center mr-3 text-lg group-hover:scale-110 transition-transform overflow-hidden shrink-0">
        ...
        <div class="mt-8">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div @click="openApp('pdfreader', 'CV - DeVanté Johnson-Rose.pdf', { pdfUrl: window.portfolioConfig.basePath + 'data/cv.pdf' })" 
                     ...
                     class="flex items-center p-2 rounded-sm hover:bg-white/20 dark:hover:bg-white/10 cursor-pointer">
                    <div class="w-8 h-8 rounded-sm bg-white flex items-center justify-center mr-3 overflow-hidden">
                ...
                <div @click="openApp('explorer', 'Projects', { initialPath: 'C:\\Users\\DeVante\\Projects' })" 
                     ...
                     class="flex items-center p-2 rounded-sm hover:bg-white/20 dark:hover:bg-white/10 cursor-pointer">
                    <div class="w-8 h-8 rounded-sm bg-yellow-100 flex items-center justify-center mr-3 overflow-hidden">
    ...
    <!-- All Apps View -->
    <div x-show="searchQuery.length === 0 && startMenuView === 'allApps'" class="grow overflow-y-auto px-8 custom-scrollbar h-full" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0 translate-x-10" x-transition:enter-end="opacity-100 translate-x-0">
        <div class="flex items-center justify-between mb-4 sticky top-0 bg-white/10 backdrop-blur-md py-2 z-10">
            ...
            <button @click="startMenuView = 'pinned'" class="text-xs bg-white/20 dark:bg-white/5 px-2 py-1 rounded-sm hover:bg-white/30 dark:hover:bg-white/10 dark:text-gray-300 transition-colors">< Back</button>
        </div>
                ...
                <div class="space-y-1">
                    ...
                    <div class="space-y-0.5">
                        <template x-for="app in group.apps" :key="app.id">
                            <div @click="openApp(app.id)" class="flex items-center p-2 rounded-sm hover:bg-white/10 cursor-pointer group transition-colors">
                                <div class="w-8 h-8 rounded-sm flex items-center justify-center mr-3 group-hover:scale-110 transition-transform overflow-hidden shrink-0">
                                    <template x-if="app.icon.includes('.')">
    ...
    <div class="h-16 bg-black/5 dark:bg-black/20 flex items-center justify-between px-8 border-t border-white/10" :class="{ 'px-4': !isDesktop }">
        <div class="flex items-center space-x-3 group cursor-pointer p-1 rounded-sm hover:bg-white/10">
        ...
        <button @click="logout()" class="p-2 rounded-sm hover:bg-white/10 group" title="Sign out">
